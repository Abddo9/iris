...
class WorldModel(nn.Module):
    def __init__(self, obs_vocab_size: int, act_vocab_size: int, config: TransformerConfig) -> None:
        ...
        self.continuous_reward = config.continuous_reward
        ...
        self.embedder = Embedder(
            ...
            embedding_tables=nn.ModuleList([nn.Embedding(act_vocab_size**self.num_agents, config.embed_dim), nn.Embedding(obs_vocab_size, config.embed_dim)])
        )
        ...
        self.head_rewards = Head(
            ...
            head_module=nn.Sequential(
                nn.Linear(config.embed_dim, config.embed_dim),
                nn.ReLU(),
                nn.Linear(config.embed_dim, (1 if self.continuous_reward else 2)*self.num_agents)
            )
        )
        ...
        self.head_ends = Head(
            ...
            head_module=nn.Sequential(
                nn.Linear(config.embed_dim, config.embed_dim),
                nn.ReLU(),
                nn.Linear(config.embed_dim, 2 * self.num_agents)
            )
        )
    ...
    def encode_actions(self, actions):
        ...
        for i in range(len(actions)):
            index += actions[len(actions)-i-1] * 5 ** i
        return index
    ...
    def compute_loss(self, batch: Batch, tokenizer: Tokenizer, **kwargs: Any) -> LossWithIntermediateLosses:
        ...
        with torch.no_grad():
            observations = rearrange(batch['observations'], 'b l t h w c -> (b l t) c h w')
        ...
        loss_rewards = F.mse_loss(rearrange(outputs.logits_rewards, 'b t e -> (b t e)'), labels_rewards.float())
        loss_ends = F.cross_entropy(rearrange(outputs.logits_ends.view(-1, self.num_agents,2), 'b t e -> (b t) e'), labels_ends)
    ...
    def compute_labels_world_model(self, obs_tokens: torch.Tensor, rewards: torch.Tensor, ends: torch.Tensor, mask_padding: torch.BoolTensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        reshaped_mask_fill = mask_fill.reshape(mask_fill.shape[0],mask_fill.shape[1],1,1).repeat(1,1,3,1)
        labels_rewards = rewards.masked_fill(reshaped_mask_fill, -100).long()
        labels_ends = ends.masked_fill(reshaped_mask_fill, -100)
